...
public class RecyclerViewPager extends RecyclerView {
    ...
    protected void onMeasure(int widthSpec, int heightSpec) {
        ...
        setPadding(size / 3, 0, size / 3, 0); //hack to get 3 items
            ...
            LinearSmoothScroller linearSmoothScroller =
                    new LinearSmoothScroller(getContext()) {
                        ...
                        protected void onTargetFound(View targetView, RecyclerView.State state, Action action) {
                            ...
                            final int time = (int) (calculateTimeForDeceleration(distance) * (distance < 720 / 2 ? 8 : 4)); //TODO this get widht of screen
                            if (time > 0) {
                                action.update(-dx, -dy, time, new DecelerateInterpolator(1.8f)); //TODO this (move out)
                            }
                        }
                    };
    ...
    public int getCurrentPosition() {
        int curPosition;
        if (getLayoutManager().canScrollHorizontally()) {
            curPosition = RecyclerViewPagerUtils.getCenterXChildPosition(this);
        } else {
            curPosition = RecyclerViewPagerUtils.getCenterYChildPosition(this);
        }
        if (curPosition < 0) {
    ...
    protected void adjustPositionX(int velocityX) {
        ...
        if (childCount > 0) {
            int curPosition = RecyclerViewPagerUtils.getCenterXChildPosition(this);
            ...
            if (targetPosition == curPosition
                ...
                View centerXChild = RecyclerViewPagerUtils.getCenterXChild(this);
    ...
    protected void adjustPositionY(int velocityY) {
        ...
        int childCount = getChildCount();
        if (childCount > 0) {
            int curPosition = RecyclerViewPagerUtils.getCenterYChildPosition(this);
            ...
            if (targetPosition == curPosition
                    && (!mSinglePageFling || mPositionOnTouchDown == curPosition)) {
                View centerYChild = RecyclerViewPagerUtils.getCenterYChild(this);
    ...
    public boolean dispatchTouchEvent(MotionEvent ev) {
        if (ev.getAction() == MotionEvent.ACTION_DOWN && getLayoutManager() != null) {
            mPositionOnTouchDown = getLayoutManager().canScrollHorizontally()
                    ? RecyclerViewPagerUtils.getCenterXChildPosition(this)
                    : RecyclerViewPagerUtils.getCenterYChildPosition(this);
        }
    ...
    public void onScrollStateChanged(int state) {
        ...
        if (state == SCROLL_STATE_DRAGGING) {
            mNeedAdjust = true;
            mCurView = getLayoutManager().canScrollHorizontally() ? RecyclerViewPagerUtils.getCenterXChild(this) :
                    RecyclerViewPagerUtils.getCenterYChild(this);
            if (mCurView != null) {
            ...
            if (mNeedAdjust) {
                int targetPosition = getLayoutManager().canScrollHorizontally() ? RecyclerViewPagerUtils.getCenterXChildPosition(this) :
                        RecyclerViewPagerUtils.getCenterYChildPosition(this);
                if (mCurView != null) {
